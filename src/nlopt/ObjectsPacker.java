...
import prototype.VPlayer;

public class ObjectsPacker {

    public static void packPlayers(ArrayList<Player> players) {

        // TODO: calculate this speed from environment parameters
        double maxDist = 2d;
        //double minDist = 1d;
        ...
        Calcfc calcfc = (int n, int m, double[] x, double[] c) -> {
            ...
            for (int i = 0; i < data.size(); i++) {
                ...
                c[cCounter++] = Math.min(maxDist, Math.sqrt(Math.pow(pd.previousPosition.x - pd.nextPosition.x, 2d) + Math.pow(pd.previousPosition.y - pd.nextPosition.y, 2d))) - Math.sqrt(Math.pow(pd.previousPosition.x - x[i * dimension], 2d) + Math.pow(pd.previousPosition.y - x[i * dimension + 1], 2d));
