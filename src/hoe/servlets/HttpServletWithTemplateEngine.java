...
public abstract class HttpServletWithTemplateEngine extends HttpServlet {
    ...
    private String concateVariableNameWithTags(String vName) {
        return HttpServletWithTemplateEngine.HTML_TAG_FROM + vName + HttpServletWithTemplateEngine.HTML_TAG_TO;
    }
    ...
    protected void appendPage(String page, HttpServletRequest request, HttpServletResponse response, String error) throws IOException {
        ...
        try (BufferedReader input = new BufferedReader(new InputStreamReader(in))) {
            ...
            while (null != (s = input.readLine())) {
                ...
                while (s.contains(HttpServletWithTemplateEngine.HTML_TAG_FROM) && s.contains(HttpServletWithTemplateEngine.HTML_TAG_TO)) {
                    String extracted = "";
                    try {
                        extracted = s.substring(HttpServletWithTemplateEngine.HTML_TAG_FROM.length() + s.indexOf(HttpServletWithTemplateEngine.HTML_TAG_FROM), s.indexOf(HttpServletWithTemplateEngine.HTML_TAG_TO));
